...
load(list_cv_path) ## An R object that contains a list of cross-validations. 
...
for(incv in names(Fold_ind_train)){
	...
	#print( table(incv.test$conditions) / sum(table(incv.test$conditions)) )
	#print( table(incv.train$conditions) / sum(table(incv.train$conditions)) )
	...
	incv.train.up <- up_train
	incv.test.up <- incv.test
	...
	#incv.train.down <- down_train
	#incv.test.down <- incv.test 
	...
	incv.train.smote <- smote_train
	incv.test.smote <- incv.test 
	...
	incv.train.rose <- rose_train
	incv.test.rose <- incv.test 


	###############################################
	########  Train the ML classifier    ##########
	###############################################
	ctrl <- trainControl(method = "repeatedcv", number = 10 , repeats = 1 , classProbs = T, summaryFunction = twoClassSummary)
	#ctrl <- trainControl(method = "LOOCV" , classProbs = T, summaryFunction = twoClassSummary)
...
#	res_df <- data.frame( Method = c("up","down","rose","smote","default"), Fold = incv , res_df)
	res_df <- rbind(up.stats,rose.stats,smote.stats,incv.stats)
	res_df <- data.frame( Method = c("up","rose","smote","default"), Fold = incv , res_df)
...
if(dim(statistics_models_best)[1] == 0){ ### In case that no single classifer satisfied the criteria, reduce the cut-off to the 50%  quantile(statistics_models$MCC,na.rm=T)[3]
...
if(dim(statistics_models_best)[1] == 0){ ### In case that no single classifer satisfied the criteria, reduce the cut-off to the 25%  quantile(statistics_models$MCC,na.rm=T)[2]
...
if(dim(statistics_models_best)[1] == 0){ ### In case that no single classifer satisfied the criteria, reduce the cut-off to the 0%  quantile(statistics_models$MCC,na.rm=T)[1]
...
for(i in indices){
	...
	####################################################################
	#### Balance according to the method that maximize the MCC #########
	####################################################################
	...
	#ctrl <- trainControl(method = "boot632", classProbs = T, summaryFunction = twoClassSummary)
...
Best_model_summary <- list(Analisis = outdir, Enterotype = ent, parametrs = c(mtry=mtry), 
	Features_selected=Features_selected, balance_method = Method, Fold = Fold,
	mean_auc = mean(stats_outer_loop_cv$AUC,na.rm=T) , mean_MCC = mean(stats_outer_loop_cv$MCC,na.rm=T,
